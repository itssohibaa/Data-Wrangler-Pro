...
# ── DATASET OVERVIEW (transformed) ────────────────────────────────────────────
st.subheader("📊 Transformed Dataset Overview")
st.info("ℹ️ The metrics below reflect the **transformed/cleaned dataset**, not the original upload.")
...
# ── TRANSFORMATION RECIPE (JSON + Python script) ───────────────────────────────
...
recipe = {
    ...
    "dataset_shape_at_export": {"rows": df.shape[0], "columns": df.shape[1]},
    "steps": [
        {
            "order": i + 1,
            "operation": s,
            "timestamp": str(datetime.now()),   # logged at export time
        }
        for i, s in enumerate(log)
    ]
}

tab_json, tab_py = st.tabs(["📦 JSON Recipe", "🐍 Python Script"])
...
with tab_py:
    st.caption("Auto-generated pandas code for the transformations applied in this session.")
    py_lines = [
        "# Auto-generated by DataWrangler Pro",
    ...
    for i, step in enumerate(log, 1):
        py_lines.append(f"# Step {i}: {step}")
    py_lines += [
        "",
        "# Save cleaned dataset",
        f"df.to_csv('{filename}.csv', index=False)",
    ]
    py_code = "\n".join(py_lines)
    st.code(py_code, language="python")
    st.download_button("⬇️ Download Script (.py)", py_code.encode(),
                       f"{filename}_pipeline.py", mime="text/plain")
...
| **Contains** | Steps + summary + timestamp | Structured JSON / Python script for automation |
    """)
